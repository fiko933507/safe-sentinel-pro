...
app.post('/api/auth/register',registerLimiter,async(req,res)=>{const p=z.object({name:z.string().min(2).max(80),email:z.string().email(),password:z.string().min(6).max(128).regex(/[A-ZÇĞİÖŞÜ]/,'Password must contain at least one uppercase letter')}).safeParse(req.body); if(!p.success){const issue=p.error.issues?.[0]; const field=String(issue?.path?.[0]||'input'); return res.status(400).json({error:'Invalid input',field,message:issue?.message||'Invalid input'});} const exists=await db.user.findUnique({where:{email:p.data.email.toLowerCase()}}); if(exists)return res.status(409).json({error:'Email already registered'}); const u=await db.user.create({data:{name:p.data.name,email:p.data.email.toLowerCase(),passwordHash:await bcrypt.hash(p.data.password,12)}}); res.status(201).json({token:await token(u,req),user:{id:u.id,name:u.name,email:u.email,status:'free'}})});
